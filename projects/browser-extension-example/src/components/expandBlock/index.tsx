...
export const ExpandBlock = (props: ExpandBlockProps) => {
    ...
    return (
        <div>
            <div style={{ transition }} className={styles.expandBlock} ref={wrapBlock}>
                {mode === 'show' ? (
                    <>
                        <ContentBlock style={{ display: isExpand === true ? 'block' : 'none' }} />
                        <FacedBlock style={{ display: isExpand === false ? 'block' : 'none' }} />
                    </>
                ) : (
