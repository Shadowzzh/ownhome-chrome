...
export class TransitionFloatFull {
    ...
    constructor(params: {
        ...
        // 计算折叠比例
        this.scale = calculateCollapsedScaleByRect(this.outRect, this.containerRect, {
            y: this.outRect.height
        })
        // 计算偏移轨迹
        this.translate = calculateCollapsedTranslateByRect(this.outRect, this.containerRect, {
            y: this.outRect.height
        })
        // 根据折叠比例，创建并设置动画
    ...
    execAnimation(params: { expand: boolean }) {
        const { expand } = params
        const { contentDom, duration, cubicBezier, outDom, upDom, enterDom, downDom } = this
